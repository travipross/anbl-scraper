def parse_price(raw_price):
    return (
        float(raw_price.lstrip("$").strip().replace(",", ""))
...
def get_product_attrs(soup):
    ...
    attrs_raw["abv_prct"] = parse_abv(attrs_raw.pop("Alcohol content", None))
    attrs_raw["quantity_per_container"] = parse_int(
        attrs_raw.pop("Quantity per container", None)
    )
    attrs_raw["container_size_ml"] = parse_vol(attrs_raw.pop("Container Size", None))
    attrs_raw["country_of_origin"] = attrs_raw.pop("Country of origin", None)
