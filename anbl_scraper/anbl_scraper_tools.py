...
def update_product_with_metadata(html_session, product, max_attempts=5):
    ...
    try:
        abv_text = (
            r.html.find(".information-attribute", containing="Alcohol content")
            .pop()
            .text
        )
    ...
    try:
        vol_text = (
            r.html.find(".information-attribute", containing="Container Size")
            .pop()
            .text
        )
    ...
    try:
        qty_text = (
            r.html.find(".information-attribute", containing="Quantity per container")
            .pop()
            .text
        )
    ...
    try:
        ...
        price_current = float(
            re.search("\$(.+) /UNIT", price_current_text).group(1).replace(",", "")
        )
    except IndexError:
        price_current = None
        success = False
    ...
    try:
        ...
        price_regular = float(
            re.search("\$(.+)", price_regular_text).group(1).replace(",", "")
        )
    except IndexError:
    ...
    try:
        ml_alcohol_per_dollar_sale = (
            float(qty) * float(vol) * (abv / 100.0) / price_current
        )
    except TypeError:
...
def url_scraper_worker(q, worker_num, stats):
    ...
    while True:
        ...
        print(
            "%4d)%4d: %s... " % (worker_num, n, (product["name"][:73] + "." * 75)[:75]),
            end="",
        )
        if success:
            print("Success")
